...
สำหรับผลลัพธ์ของ transaction ที่ใช้เวลานาน API process จะส่งคืนสมาชิกทาง callback ที่สมาชิกประกาศไว้กับ process
...
- ตรวจสอบรูปแบบ transaction ที่ทำลง blockchain
- คำนวณ hash ของ data ณ ปัจจุบันเพื่อเปรียบเทียบกับสมาชิกในวง
...
ระบบนี้เริ่มต้นออกแบบด้วยการแบ่งระดับความปลอดภัย/ความสะดวก 3 ระดับ

Mode 1
  - ความปลอดภัยน้อยที่สุด
  - IdP ไม่ต้องได้รับอนุญาตจาก user เพื่อได้รับสิทธิในการยืนยันตัวตน
    - แค่ user เป็นลูกค้าของ IdP นั้นๆ IdP ก็สามารถยืนยันได้
...
Mode 2
  ...
  - IdP ไม่ต้องได้รับอนุญาตจาก user เพื่อได้รับสิทธิในการยืนยันตัวตน
    - แค่ user เป็นลูกค้าของ IdP นั้นๆ IdP ก็สามารถยืนยันได้
  - user ต้องมีการประกาศเลือก IdP ที่จะได้รับการแจ้งเตือน
  - RP เลือกว่าจะให้ user ยืนยันด้วย IdP ไหน
  - ต่างจาก mode 1 ที่ธุรกรรมที่เกิดขึ้นต้อง "แจ้งเตือน" ไปยัง IdP ที่ user ได้ประกาศเลือกเอาไว้บนระบบ
    - แต่อาจจะได้รับคำยืนยันจาก IdP อื่น

Mode 3
  - ความปลอดภัยมากที่สุด
  - IdP *ต้อง*ได้รับอนุญาตจาก user เพื่อได้รับสิทธิในการยืนยันตัวตน
  - RP เลือกว่าจะให้ user ยืนยันด้วย IdP ไหน แต่ต้องเป็น IdP ที่ user อนุญาตแล้วเท่านั้น
  - ต่างจาก mode 2 ที่ธุรกรรมที่เกิดขึ้นต้อง "ได้รับคำยืนยัน" จาก IdP ที่ user ได้ประกาศเลือกเอาไว้บนระบบ

ในขั้นต้นระบบจะเปิดใช้ด้วย mode 1 แต่บริษัท NDID จะผลักดันให้สมาชิกๆค่อยๆโอนย้ายไปยัง mode 3 ให้หมด
และยกเลิกการรองรับ mode 1 ในที่สุด

ส่วน mode 2 ปัจจุบันที่ประชุมลงความเห็นว่าไม่ควรรองรับ เนื่องจากความสะดวกไม่ต่างกับ mode 3 มากนัก
แต่ความปลอดภัยลดลง
